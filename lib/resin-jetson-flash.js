	...
	constructor(deviceType, image, odmdata, assetDir, output, cache) {
		...
		this.dictionary = {
			...
			['jetson-xavier-nx-devkit-tx2-nx']: {
                                url:
                                        'http://127.0.0.1/Jetson_Linux_R32.7.2_aarch64.tbz2',
                                partitions: {
						kernel: { path: null },
						kernel_b: { path: null },
						'bootloader-dtb': { path: null },
	...
	spawnFlash() {
		return new Bluebird(async (resolve, reject) => {
			...
			if ("jetson-xavier-nx-devkit-tx2-nx" == this.deviceType.toString()) {
				try {
					fs.copyFileSync(path.resolve(__dirname, '../Config_Files_For_WBT/tegra186-mb1-bct-pinmux-p3636-0001-a00_wbt.cfg'),
								workPath + '/bootloader/t186ref/BCT/tegra186-mb1-bct-pinmux-p3636-0001-a00_wbt.cfg');
					const tx2NxConfFile = join(workPath, '/p3636.conf.common');
					var tx2NxConf = fs.readFileSync(tx2NxConfFile, 'utf8');
					var replacedPinmux = tx2NxConf.replace(/tegra186-mb1-bct-pinmux-p3636-0001-a00.cfg/g, 'tegra186-mb1-bct-pinmux-p3636-0001-a00_wbt.cfg');
					fs.writeFileSync(tx2NxConfFile, replacedPinmux, 'utf8');
					console.log("Updated WBT pinmux cfg in p3636.conf.common");
				} catch (err) {
					console.error(err);
				}
			}
