	...
	async generateArtifacts() {
		...
		if ((!(await utils.checkConsistency(nvidiaToolPath)) && !this.cache)) {
			...
			console.log('Using cached Linux_for_Tegra');
               ...
               // TODO: Extend this to rest of devices
               if (this.deviceType === 'jetson-agx-orin-devkit') {
                       fs.copyFileSync(join(nvidiaToolPath + '/bootloader/t186ref/cfg/flash_t234_qspi_sdmmc.xml'), join(this.assetDir, 'flash_t234_qspi_sdmmc.xml'));
                       console.log("Copied " + join(nvidiaToolPath + '/bootloader/t186ref/cfg/flash_t234_qspi_sdmmc.xml') + ' to ' + join(this.assetDir, 'flash_t234_qspi_sdmmc.xml'));
