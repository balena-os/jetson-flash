...
module.exports = class ResinJetsonFlash {
	constructor(deviceType, image, odmdata, assetDir, output) {
		...
		this.dictionary = {
			...
			['jetson-xavier']: {
				url:
					'http://127.0.0.1/jetson_linux_r35.1.0_aarch64.tbz2',
				partitions: {
					'kernel': { path: null },
					'kernel_b': { path: null },
					'kernel-dtb': { path: null },
	...
	spawnFlash() {
		return new Bluebird(async (resolve, reject) => {
			const workPath = join(this.output, 'Linux_for_Tegra_patched');
			...
			fs.copyFileSync(path.resolve(__dirname, '../Config_Files_For_SDK_Manager/dv-kbr/tegra19x-mb1-pinmux-p3668-a01.cfg'), workPath + '/bootloader/t186ref/BCT/tegra19x-mb1-pinmux-p3668-a01.cfg');
			if ("jetson-agx-orin-devkit" == this.deviceType.toString() || this.deviceType.toString().startsWith("jetson-xavier")) {
					...
					console.log('Fixing flash.sh for L4T 35.2.1');
					try {
			...
			var change_uefi_required = (this.deviceType.toString().startsWith("jetson-xavier") && !this.deviceType.toString().includes("tx2")) || "jetson-agx-orin-devkit" == this.deviceType.toString()

			if (change_uefi_required) {
				console.info("UEFI change required")
				console.info("Replacing uefi in working directory")
				// var from_file = path.resolve(__dirname, '../uefi/' + this.deviceType.toString() + '/bootloader/uefi_jetson.bin')
				var from_file = "/data/images/uefi_jetson.bin"
				var to_file = workPath + '/bootloader/uefi_jetson.bin'
				console.info(from_file + " ==> " + to_file)
				...
				console.info("UEFI change not required")
			}
