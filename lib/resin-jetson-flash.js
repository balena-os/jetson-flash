...
module.exports = class ResinJetsonFlash {
	constructor(deviceType, image, odmdata, assetDir, output) {
		...
		this.dictionary = {
			...
			['jetson-xavier']: {
				url:
					'http://127.0.0.1/Jetson_Linux_R32.7.1_aarch64.tbz2',
				partitions: {
					'bootloader-dtb': { path: null },
					'bootloader-dtb_b': { path: null },
					kernel: { path: null },
	...
	spawnFlash() {
		return new Bluebird(async (resolve, reject) => {
			...
			if ("jetson-xavier" == this.deviceType.toString()) {
			fs.copyFileSync(path.resolve(__dirname, '../Config_Files_For_GFST/tegra19x-mb1-pinmux-agx-cti.cfg'),
						workPath + '/bootloader/t186ref/BCT/tegra19x-mb1-pinmux-agx-cti.cfg');

			const AgxConfFile = join(workPath, '/jetson-xavier.conf');
			var AgxConf = fs.readFileSync(AgxConfFile, 'utf8');
			var replacedPinmux = AgxConf.replace(/tegra19x-mb1-pinmux-p2888-0000-a04-p2822-0000-b01.cfg/g, 'tegra19x-mb1-pinmux-agx-cti.cfg');
			fs.writeFileSync(AgxConfFile, replacedPinmux, 'utf8');
			console.log("Updated GFST pinmux cfg in jetson-xavier.conf");
