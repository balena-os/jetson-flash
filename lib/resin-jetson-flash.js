...
module.exports = class ResinJetsonFlash {
	...
	spawnFlash() {
		return new Bluebird(async (resolve, reject) => {
			...
			if ("jetson-agx-orin-devkit" == this.deviceType.toString()) {
					fs.copyFileSync(path.resolve(__dirname, '../AGX_Orin/bootloader/uefi_t23x_general.bin'), workPath + '/bootloader/uefi_bins/uefi_t23x_general.bin');
					...
					var dtsi = join(workPath, '/bootloader/tegra234-mb2-bct-common.dtsi');
					var dtsiData = fs.readFileSync(dtsi, 'utf8');
					var replacedContents = dtsiData.replace(/cvb_eeprom_read_size = <0x100>;/g, 'cvb_eeprom_read_size = <0x0>;');
	...
	async generateArtifacts() {
		...
		if ((!(await utils.checkConsistency(nvidiaToolPath)) && !this.cache)) {
			...
			try {
				execFileSync('find', [nvidiaToolPath, '-xtype', 'l', '-delete']);
			} catch (err) {
				console.warn('Warning: Failed to clean broken BSP symlinks:', err.message);
			}
