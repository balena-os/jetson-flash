...
module.exports = class ResinJetsonFlash {
	constructor(deviceType, image, odmdata, assetDir, output) {
		...
		this.dictionary = {
			...
			['jetson-xavier-nx-devkit-emmc']: {
                                url:
                                        'http://127.0.0.1/Jetson_Linux_R35.2.1_aarch64.tbz2',
                                partitions: {
                                        kernel: { path: null },
                                        kernel_b: { path: null },
                                        'kernel-dtb': { path: null },
	...
	spawnFlash() {
		return new Bluebird(async (resolve, reject) => {
			...
			if ((this.deviceType.toString().startsWith("jetson-xavier") && !this.deviceType.toString().includes("tx2")) || "jetson-agx-orin-devkit" == this.deviceType.toString() ) {
				try {
					fs.copyFileSync(path.resolve(__dirname, '../uefi/' + this.deviceType.toString() + '/bootloader/uefi_jetson.bin'), workPath + '/bootloader/uefi_jetson.bin');
				} catch (err) {
					console.error(err);
				}
			}
