...
module.exports = class ResinJetsonFlash {
	...
	spawnFlash() {
		return new Bluebird(async (resolve, reject) => {
			...
			if ("jetson-agx-orin-devkit" == this.deviceType.toString() || "jetson-xavier" == this.deviceType.toString()) {
			...
			if (this.deviceType.toString().startsWith("jetson-xavier")) {
				try {
					fs.copyFileSync(path.resolve(__dirname, '../AGX/bootloader/uefi_jetson.bin'), workPath + '/bootloader/uefi_jetson.bin');
				} catch (err) {
					console.error(err);
				}
			}
