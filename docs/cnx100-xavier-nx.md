...
```sh
$ ./bin/cmd.js -f <balena.img> -m cnx100-xavier-nx
```
...
```sh
./bin/cmd.js -f /data/images/<balena.img> -m cnx100-xavier-nx --accept-license=yes -c /tmp/Linux_for_Tegra
```

You can alternatively just run the jetson-flash tool in a single command by running the container with this command:

```sh
docker container run --rm -it --privileged -v /dev/bus/usb:/dev/bus/usb -v ~/images:/data/images jetson-flash-image ./bin/cmd.js -f /data/images/<balena.img> -m cnx100-xavier-nx --accept-license=yes -c /tmp/Linux_for_Tegra
```
