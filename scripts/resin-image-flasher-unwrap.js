...
const getResinImageFromFlasher = async (image, output) => {
  ...
  const size = await Bluebird.using(resinfs.interact(image, RESIN_FLASHER.rootfsID), async (fs) => {
    const path = join(RESIN_FLASHER.resinImagePath,
...
const copyResinConfigurationOver = async (flasherImage, resinImage) => {
  ...
  const createFSH = (options) => {
    ...
    return Bluebird.using(resinfs.interact(options.image, options.partition), (fs) => {
  ...
  const performCopy = async (paths) => {
    await Bluebird.each(paths, async (path) => {
      await Bluebird.using(resinfs.interact(resinImage, resinImageBootPartID), async (fs) => {
  ...
  console.log(`Copying resin configuration from ${flasherImage} to ${resinImage}`)
  const toCopy = await Bluebird
    .map(paths, (path) => {
      return createFSH({
        image: flasherImage,
        partition: RESIN_FLASHER.bootID,
